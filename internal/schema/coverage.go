...
var Excluded = map[OperationKey]string{
	...
	// New in API v1.1.0 — agent-session and machine-event infrastructure.
	{Method: "PATCH", Path: "/api/credential_device_authorizations/:id"}:    "device-auth pod internal; not an account API token endpoint",
	{Method: "GET", Path: "/api/events"}:                                    "machine-event replay; not yet implemented in loopctl",
	{Method: "GET", Path: "/api/tasks/:task_id/events"}:                     "task machine-event replay; not yet implemented in loopctl",
	{Method: "POST", Path: "/api/containers/:container_id/agent_sessions"}:  "container-internal: agent container registers its own session",
	{Method: "PUT", Path: "/api/agent_sessions/:id"}:                        "container-internal: agent session lifecycle management",
	{Method: "PATCH", Path: "/api/agent_sessions/:id"}:                      "container-internal: agent session lifecycle management",
	{Method: "PATCH", Path: "/api/agent_sessions/:agent_session_id/stream"}: "container-internal: agent session transcript stream",
	{Method: "PUT", Path: "/api/agent_sessions/:agent_session_id/stream"}:   "container-internal: agent session transcript stream (PUT alias)",
